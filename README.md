...
Dialog fields: Supplier, Sample Qty Needed, Artwork Receive Date, SRD, Sample
Time, Sample Type. Supplier is a dropdown fed from the **Factories** list on
the same site, plus any name you've typed before, and free text is always
allowed. Sample Type defaults to Artwork Sample.
...
factory as-is. Style#, Description, and Notes come from the row; the rest come
from the dialog. Rows with no photo export with a blank Picture cell, and the
