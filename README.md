...
## Export to Development Tracker

**Export to Dev Tracker** writes selected rows as an .xlsx in the Development
Tracker's column order: Status, Date Created, Supplier, PO#, Quote#,
SR / BF / BS / FF #, PPS Comment, Sample Type, Customer, Style #, Item
Description.

Mapped from the tracker: Date Created, Customer, Style #, Item Description.
Status defaults to `Open`. Everything else exports blank to fill in Excel.

Selecting rows:
- Click a row number, or shift-click, for a contiguous block
- **Ctrl+click** row numbers to tag any set of rows; tagged rows highlight and
  the button shows the count
- With nothing tagged, the export uses whatever rows the cell selection covers

Changing a filter drops tagged rows that are no longer visible, so a hidden
selection can't be exported by accident.

Output has bold centered headers on the salmon fill, thin borders, autofilter,
a frozen header row, `m/d/yyyy` dates as real date values, and landscape
fit-to-width page setup. Paste the rows straight into the tracker, or keep the
file as the batch record.

## Sample Request (factory-ready)

**Sample Request** builds the Sample_Request_Template layout from the selected
rows: Supplier, Picture, Style#, Description, Sample Qty Needed, Artwork
Receive Date, SRD, Sample Time, Notes.

Product photos are embedded in the Picture column at true aspect ratio and
centered, so the file goes to the factory as-is with no separate attachments.
Style#, Description, and Notes come from the row.

Before building, a short dialog collects the fields that apply to the whole
request — Supplier, Sample Qty Needed, Artwork Receive Date, SRD, Sample Time
— and writes the same value down every row. All are optional; leave any blank
to fill in Excel. The filename uses the supplier name when given.

Image bytes are pulled from SharePoint (pre-authenticated link first, then the
Graph content endpoint, then the repo folder). Rows with no photo still export
with a blank Picture cell, and the dialog says up front how many that will be.

Dates land as real Excel date values formatted `m/d/yyyy`. Page setup is
landscape fit-to-width for printing.
